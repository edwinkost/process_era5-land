...
bash remapcon_arise.sh ${INP_FILE} ${RESOLUTION} ${OUT_FOLDER}/${OUTP_FILE} ${OUTP_FILE}_rempacon-30-arcmin  ${CDO_MON_METH} ${CDO_MON_UNIT} ${CLONE30SEC_FILE}
...
bash remapcon_arise.sh ${INP_FILE} ${RESOLUTION} ${OUT_FOLDER}/${OUTP_FILE} ${OUTP_FILE}_rempacon-5-arcmin   ${CDO_MON_METH} ${CDO_MON_UNIT} ${CLONE30SEC_FILE}

#~ # for Europe we can skip the following
...
#~ bash remapcon_arise.sh ${INP_FILE} ${RESOLUTION} ${OUT_FOLDER}/${OUTP_FILE} ${OUTP_FILE}_rempacon-150-arcsec ${CDO_MON_METH} ${CDO_MON_UNIT} ${CLONE30SEC_FILE}
